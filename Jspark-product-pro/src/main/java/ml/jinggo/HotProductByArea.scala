...
object HotProductByArea {
  def main(args: Array[String]): Unit = {
    ...
    //获取地区数据
    val areaDF = spark.sparkContext.textFile("hdfs://192.168.1.222:9000/project4/areaf/info")
                  .map(_.split(",")).map(x=> new AreaInfo(x(0),x(1))).toDF()
    areaDF.createTempView("area")

    //获取商品数据
    val productDF = spark.sparkContext.textFile("hdfs://192.168.1.222:9000/project4/product/info")
      .map(_.split(",")).map(x=> new ProductInfo(x(0),x(1)))
      .toDF()
    productDF.createTempView("product")

    //获取点击日志
    val clickLogDF = spark.sparkContext.textFile("hdfs://hdp21:8020：9000/project4/clicklog/info")
      .map(_.split(",")).map(x => new LogInfo(x(0),x(1),x(2).substring(x(2).indexOf("=")+1),x(3),x(4),x(5)))
      .toDF()
    clickLogDF.createTempView("clicklog")

    //执行SQL
    val sql = "select a.area_id,a.area_name,p.product_id,product_name," +
      "count(c.product_id) from area a,product p,clicklog c where " +
      "a.area_id=c.area_id and p.product_id=c.product_id group by" +
      " a.area_id,a.area_name,p.product_id,p.product_name"
