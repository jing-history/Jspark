...
object ProductSalesInfo {
  def main(args: Array[String]): Unit = {
    ...
    //第一步：取出商品的数据
    val productInfo = sc.textFile("hdfs://192.168.1.222:8020/order/products").map(line=>{
    ...
    //第二步：取出订单的数据
    val orderInfo = sc.textFile("hdfs://192.168.1.222:8020/order/sales").map(line=>{
      val words = line.split(",")

      //返回： 商品的ID、年份、金额
      (
        Integer.parseInt(words(0)),
        Integer.parseInt(words(2).substring(0,4)),
    ...
    //第二步：将行转成列
    sQLContext.sql("select prod_name,sum(case year_id when 1998 then total else 0 end)," +
      "sum(case year_id when 1999 then total else 0 end)," +
      "sum(case year_id when 2000 then total else 0 end)," +
      "sum(case year_id when 2001 then total else 0 end) " +
      "from result group by prod_name").show
